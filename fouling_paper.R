...
packages <- c("tidyverse", "glmmTMB", "emmeans", "performance", "plotrix",
              "MASS", "DHARMa", "arm", "grDevices","geoR", "rstatix", "PNWColors",
              "broom.mixed", "broom.helpers", "gt", "marginaleffects", "zoo")
...
hobo <- read.csv("./data/hobo.csv") %>% 
  mutate(date=mdy_hms(Date.time),
         temp=Temp, sal=High.sal,
         loc=Location,.keep="unused")

#Excludes observations where the temperature was erroneously high due to air exposure
#i.e., the sensor was measuring air temperature instead of water temperature
...
common <- inner_join(Inside, Outside, by="date") %>% 
  ...
  mutate(temp_diff=temp_in-temp_out) #find the difference between the temperature inside and outside the pound
...
noAir_roll <- noAir %>% group_by(loc) %>% 
  mutate(across(where(is.double), \(x) rollmean(x, k = 24, fill = NA))) %>% 
  na.omit()
...
#Import data
ic_fall <- read.csv("./data/accel/Inner_Cage_Accel_9.12.22to10.6.22.csv") %>% mutate(trt="ic") #Inside cages
ib_fall <- read.csv("./data/accel/Inner_FB_Accel_9.12.22to10.6.22.csv") %>% mutate(trt="ib") #Inside bags
oc_fall <- read.csv("./data/accel/Outer_Cage_Accel_9.12.22to10.6.22.csv") %>% mutate(trt="oc") #Outside cages

accel_dfs <- list(ic_fall, ib_fall, oc_fall)

#This function converts the time column into the correct format, renames variables, and selects only the relevant columns
clean_accel_df <- function(df) {
  df %>% mutate(date=ymd_hms(ISO.8601.Time),
                accelX=Ax..g.,
                accelY=Ay..g.,
                accelZ=Az..g.) %>% 
...
#This applies the clean_accel_df function to all three data frames in the accel_dfs list
...
types_filled <- right_join(types %>% group_by(date, trt, gear, location) %>% 
                             mutate(oyster_num = row_number()), grid) %>% 
...
mod <- glmmTMB(prop_fouling ~ date*gear*loc, data = ratios, family = beta_family())

# Checking other formulations - commented out because these take a while to run
...
# Commented out because these take a while to run
